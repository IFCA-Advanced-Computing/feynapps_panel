...
class FeynApps:
    def __init__(self):
        try:
            self.config = json.loads(urllib2.urlopen(APPS_URL).read())
        except Exception, e:
...
class PostCreationStep(workflows.Step):
    ...
    t = [ 'feynapp_context_%s' % a for a in FeynApps().get_apps()] 
    contributes = tuple(t)
...
class CustomizeInstance(workflows.Workflow):
